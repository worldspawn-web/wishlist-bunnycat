...
export default function AddWishModal({ isOpen, onClose, onAdd }: AddWishModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newWish: Omit<Wish, 'id' | 'createdAt'> = {
      title,
      image,
    ...
    try {
      await onAdd(newWish);
      onClose();
      setNotification({ message: 'Желание успешно добавлено', type: 'success' });
      // Reset form
      setCategory('');
      setTitle('');
      setImage('');
      ...
      setNotification({ message: 'Не удалось добавить желание. Пожалуйста, попробуйте еще раз.', type: 'error' });
    } finally {
